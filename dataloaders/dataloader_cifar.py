        ...
        if self.mode == "test":
            ...
            if self.mode == "all":
                ...
                if oracle == "negative":
                    ...
                    noise_label[pred_clean] = np.random.randint(
                        0, self.num_classes, len(noise_label[pred_clean])
                    )
                ...
                if self.mode == "labeled":
                    ...
                    tp, fp, fn = (
                        (np.equal(pred, clean) * (clean == 1)).sum(),
...
class cifar_dataloader:
    def __init__(
        ...
        self.transform_warmup = (
            self.transform_warmup if stronger_aug else self.transform_train
        )
        self.clean = cifar_dataset(
